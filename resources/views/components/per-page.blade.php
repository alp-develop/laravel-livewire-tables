...
>
    ...
    >
        ...
        <svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 20 20" fill="currentColor" style="width:14px;height:14px;color:#9ca3af;flex-shrink:0">
            <path fill-rule="evenodd" d="M5.22 8.22a.75.75 0 0 1 1.06 0L10 11.94l3.72-3.72a.75.75 0 1 1 1.06 1.06l-4.25 4.25a.75.75 0 0 1-1.06 0L5.22 9.28a.75.75 0 0 1 0-1.06z" clip-rule="evenodd" />
        </svg>
    </button>
    <div x-show="open" x-cloak x-transition @click.stop
        style="position:absolute;right:0;top:100%;z-index:9999;margin-top:0.25rem;background:white;border:1px solid #e5e7eb;border-radius:0.5rem;box-shadow:0 4px 6px -1px rgba(0,0,0,.1);min-width:4.5rem;overflow:hidden"
    >
        @foreach($this->getPerPageOptions() as $option)
            <div
                ...
                :style="$wire.perPage == {{ $option }}
                    ? 'background:var(--lt-primary-200,#d1d5db);color:var(--lt-primary-700,#374151);font-weight:600'
                    : (hovered == {{ $option }} ? 'background:var(--lt-primary-50,#f3f4f6)' : '')"
            >

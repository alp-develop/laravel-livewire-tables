...
<div
            ...
            let boot = () => {
                ...
                let config = {
                    ...
                    onChange: (dates) => {
                        if (dates.length === 1) {
                            this.hasValue = true;
                            this.$wire.set('tableFilters.{{ $filter->getKey() }}', flatpickr.formatDate(dates[0], 'Y-m-d'));
                        } else {
                            this.hasValue = false;
                ...
                if (stored) { this.instance.setDate(stored); this.hasValue = true; }
            };
    ...
    x-on:remove-filter.window="if ($event.detail && $event.detail.field === '{{ $filter->getKey() }}' && instance) { instance.clear(); hasValue = false; $wire.set('tableFilters.{{ $filter->getKey() }}', '') }"
    x-on:livewire-tables:clear-filters.window="if (instance) { instance.clear(); hasValue = false; } $wire.set('tableFilters.{{ $filter->getKey() }}', '')"
...
>
    ...
    <input
        ...
        placeholder="{{ $filter->getPlaceholder() ?: __('livewire-tables::messages.select_option') }}"
        readonly
    />
    <button
        type="button"
        x-show="hasValue"
        x-cloak
        x-on:click.stop="if (instance) instance.clear(); hasValue = false; $wire.set('tableFilters.{{ $filter->getKey() }}', '')"
